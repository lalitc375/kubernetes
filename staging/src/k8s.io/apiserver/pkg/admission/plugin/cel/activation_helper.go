...
func createAdmissionRequestValue(request *admissionv1.AdmissionRequest, objectVal, oldObjectVal any) (map[string]interface{}, error) {
	...
	// Empty metav1.CreateOptions and metav1.DeleteOptions resolve to an empty map
	// in ToUnstructured(), so we must explicitly omit the "options" key if the resulting map is empty
	// to match the omitempty JSON serialization behavior that CEL policies expect.
